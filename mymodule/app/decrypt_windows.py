...
class DecryptWindows:
    ...
    def decrypt_file_windows(self):
        def __check_password__():
            ...
            try:
                self.copy_files()
                self.string_password = password.get()
                check_code = self.decrypt_file(string_password=self.string_password) and self.config_file.read()['verbose']
                if check_code:
                    messagebox.showinfo(
                        message='Files decrypted correctly', title='Decrypted'
                    )
                self.kill_root_windows()
                return True, self.string_password
            except:
                self.create_error_windows(
                    title='Decrypt Error', error_text='Error decrypt files'
                )
                return False
    ...
    def decrypt_file(self, string_password: str, files: list = None):
        if not self.config_file.read()['check_code']: return True
        try:
            if files is None: files = self.config_file.read()['files']
            CRYT = crypto.CRYPTO()
            for file in files:
                crypto.write_file(CRYT.decrypt_string(
                    string=crypto.content_file(file=file),
                    password=bytes(string_password, encoding='utf-8')
                ), file)
            config = self.config_file.read()
            config['check_code'] = False
            self.update_config_file(config)
            self.decrypt_file_status = not config['check_code']
            return not config['check_code']
        except:
            return False
    ...
    def get_value(self):
        return self.decrypt_file_status, self.string_password
    ...
    def create_error_windows(title: str = None, geometry='500x120', error_text: str = None):
        ...
        frame_width = eval(width_height[0]) - 40;
        frame_height = eval(width_height[1]) - 70
        frame = Frame(
...
# main function
def main():
    app = DecryptWindows(config_file='/opt/SecureBrowser/data/config.json')
    app.decrypt_file_windows()
    app.mainloop()
    status, string_password = app.get_value()
    return status, string_password
